...
import Day from "./Day";
import cuuid from "../../assets/utils/cuuid.jsx";
...
class Week extends Component {
	render() {
		...
		for (var i = 0; i < 7; i++) {
			...
			for (var j = 0; j < selectedMonthEvents.length; j++) {
				if (selectedMonthEvents[j].date.isSame(date, "day")) {
					...
					title = selectedMonthEvents[j].title
						...
						: "";
					info = selectedMonthEvents[j].info
						? selectedMonthEvents[j].info
						: "";
					itemStyle = selectedMonthEvents[j].itemStyle
						? selectedMonthEvents[j].itemStyle
						: "";
					link = selectedMonthEvents[j].link
						? selectedMonthEvents[j].link
						: "";
				}
			}
			...
			days.push(<Day key={cuuid()} day={day} selected={selected} />);
			date = date.clone();
			date.add(1, "d");
		}
